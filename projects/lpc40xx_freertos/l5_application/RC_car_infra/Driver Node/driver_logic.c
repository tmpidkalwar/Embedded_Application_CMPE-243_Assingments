...
const static float sensor_to_steer_angle_map_const = (max_angle - min_angle) /
    (max_sensor_value_to_start_steering - min_sensor_value_to_map_max_steering_angle);
...
static void steer_signal_to_avoid_obstacle(dbc_DRIVER_TO_MOTOR_s * motor_signals) {
    if(sensor_recvd_values.SENSOR_SONARS_left < sensor_recvd_values.SENSOR_SONARS_right) {
        ...
        if(sensor_recvd_values.SENSOR_SONARS_left < max_sensor_value_to_start_steering) {
            motor_signals->MOTOR_direction = (int8_t)(sensor_to_steer_angle_map_const
                * sensor_recvd_values.SENSOR_SONARS_left);
        }
        ...
        if(sensor_recvd_values.SENSOR_SONARS_right < max_sensor_value_to_start_steering) {
            motor_signals->MOTOR_direction = (int8_t)(-sensor_to_steer_angle_map_const
                 * sensor_recvd_values.SENSOR_SONARS_right);
        }
...
const static float sens_val_to_fwd_speed_map_const = 
    (fwd_sens_val_to_map_to_max_fwd_speed - fwd_sens_val_to_start_fwd_moving_car) /
