...
/// @{ ISR functions for all UARTs
static void uart0_isr(void);
static void uart1_isr(void);
static void uart2_isr(void);
static void uart3_isr(void);
static void uart__handle_interrupt(uart_s *uart_type); ///< Common function for all UART ISRs
/// @}
...
static bool uart__load_more_bytes_to_transmit(uart_s *uart_type) {
...
static void uart0_isr(void) { uart__handle_interrupt(&uarts[UART__0]); }
static void uart1_isr(void) { uart__handle_interrupt(&uarts[UART__1]); }
static void uart2_isr(void) { uart__handle_interrupt(&uarts[UART__2]); }
static void uart3_isr(void) { uart__handle_interrupt(&uarts[UART__3]); }

static void uart__handle_interrupt(uart_s *uart_type) {
  bool context_switch_required = false;
  ...
  case transmitter_empty:
    context_switch_required = uart__load_more_bytes_to_transmit(uart_type);
    break;
...
void uart__init(uart_e uart, uint32_t peripheral_clock, uint32_t baud_rate) {
  if (!uart__is_enabled(uart)) {
    ...
    uart_regs->DLM = (divider >> 8);
    uart_regs->DLL = (divider >> 0);
...
bool uart__polled_put(uart_e uart, char output_byte) {
  bool status = false;
  lpc_uart *uart_mmap = uarts[uart].registers;
  ...
  if (uart__is_enabled(uart)) {
    ...
    if (rtos_is_running && queue_is_enabled) {
      ...
      uart__wait_for_transmit_to_complete(uart_mmap);
      uart_mmap->THR = output_byte;
    }
...
bool uart__get(uart_e uart, char *input_byte, uint32_t timeout_ms) {
  ...
  if (uart__is_receive_queue_enabled(uart) && rtos_is_running) {
    QueueHandle_t q_handle = uarts[uart].queue_receive;
    status = xQueueReceive(q_handle, input_byte, uart__ms_to_os_ticks(timeout_ms));
  }
...
bool uart__put(uart_e uart, char output_byte, uint32_t timeout_ms) {
  ...
  if (uart__is_receive_queue_enabled(uart) && rtos_is_running) {
    QueueHandle_t transmit_queue = uarts[uart].queue_transmit;

    // Deposit to the transmit queue for now
    status = xQueueSend(transmit_queue, &output_byte, uart__ms_to_os_ticks(timeout_ms));
...
bool uart__is_enabled(uart_e uart) {
  lpc_uart *regs = uarts[uart].registers;
  return (0 != regs->LCR);
}
